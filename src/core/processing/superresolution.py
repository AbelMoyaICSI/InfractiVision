...
def enhance_plate_image(plate_bgr, is_night=False):
    """
    Función wrapper para compatibilidad con el código que busca enhance_plate_image
    """
    try:
        # Si es de noche, aplicar procesamiento adicional
        if is_night:
            # Para modo nocturno, aplicar un poco más de realce
            enhanced = enhance_plate(plate_bgr)
            
            # Aplicar un filtro adicional para condiciones nocturnas
            gray = cv2.cvtColor(enhanced, cv2.COLOR_BGR2GRAY)
            
            # Realce adicional para condiciones nocturnas
            bright = cv2.convertScaleAbs(gray, alpha=1.2, beta=20)
            
            # Convertir de vuelta a BGR
            return cv2.cvtColor(bright, cv2.COLOR_GRAY2BGR)
        else:
            # Para modo diurno, usar el procesamiento estándar
            return enhance_plate(plate_bgr)
