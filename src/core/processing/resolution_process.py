...
def enhance_plate_image(plate_img, is_night=False, output_path=None, region="ES"):
    """
    MODO MASTER FINAL: Preservamos el color y textura natural.
    """
    if plate_img is None or plate_img.size == 0:
        return None

    # Redimensionar suavemente si es muy pequeña
    h, w = plate_img.shape[:2]
    if h < 40:
...
def get_common_plate_patterns(region):
    ...
    MEJORA: Patrones actualizados del compañero con soporte mejorado para España
    """
    patterns = {
        # Patrones de placas españolas (ACTUALIZADOS CON MEJORAS DEL COMPAÑERO)
        "ES": {
            # Formatos principales de placas españolas
            "pattern": r"^\d{4}[BCDFGHJKLMNPRSTVWXYZ]{3}$",  # 1234XXX (formato actual)
            "old_pattern": r"^[A-Z]{1,2}\d{4}[A-Z]{2}$",      # XX1234XX (formato antiguo)
            
            # NUEVOS: Patrones adicionales específicos encontrados en España
            "regional_patterns": [
                r"^[A-Z]\d{4}[A-Z]{2}$",     # A1234BC (formato regional)
                r"^[A-Z]{2}\d{3}[A-Z]$",     # AB123C (formato especial)
                r"^[A-Z]{3}\d{3}$",          # ABC123 (formato corto)
                r"^[A-Z]\d{3}[A-Z]{2}$",     # A123BC (formato mixto)
                r"^[A-Z]{2}\d{4}$",          # AB1234 (formato numérico)
                r"^\d{3}[A-Z]{3}$",          # 123ABC (formato inverso)
                r"^[A-Z]\d{2}[A-Z]\d{2}$",   # A12B34 (formato alternativo)
                # Formatos con guiones (frecuentes en OCR)
                r"^\d{4}-[A-Z]{3}$",         # 1234-ABC
                r"^[A-Z]{2}-\d{4}$",         # AB-1234
                r"^[A-Z]\d{4}-[A-Z]{2}$",    # A1234-BC
                # Formatos con espacios (frecuentes en OCR)
                r"^\d{4}\s[A-Z]{3}$",        # 1234 ABC
                r"^[A-Z]{2}\s\d{4}$",        # AB 1234
                r"^[A-Z]\d{4}\s[A-Z]{2}$",   # A1234 BC
                # Formatos motocicletas y ciclomotores
                r"^\d{4}[A-Z]{2}$",          # 1234AB (motos)
                r"^[A-Z]\d{4}[A-Z]$",        # A1234B (motos)
            ],
            
            # MEJORADO: Confusiones de caracteres más exhaustivas
            "character_confusions": {
                # Números confundidos con letras
                "0": "ODCQG",   # 0 confundido con O, D, C, Q, G
                "1": "ILTJ7",   # 1 confundido con I, L, T, J, 7
                "2": "ZS",      # 2 confundido con Z, S
                "3": "E8B",     # 3 confundido con E, 8, B
                "4": "AH",      # 4 confundido con A, H
                "5": "S",       # 5 confundido con S
                "6": "GC",      # 6 confundido con G, C
                "7": "T1J",     # 7 confundido con T, 1, J
                "8": "BR3",     # 8 confundido con B, R, 3
                "9": "GQ",      # 9 confundido con G, Q
                
                # Letras confundidas con números
                "A": "4H",      # A confundido con 4, H
                "B": "8R3",     # B confundido con 8, R, 3
                "C": "G06",     # C confundido con G, O, 6
                "D": "0O",      # D confundido con 0, O
                "E": "F3",      # E confundido con F, 3
                "F": "PE",      # F confundido con P, E
                "G": "C6O9",    # G confundido con C, 6, O, 9
                "H": "4A",      # H confundido con 4, A
                "I": "1LTJ7",   # I confundido con 1, L, T, J, 7
                "J": "I1T",     # J confundido con I, 1, T
                "K": "X",       # K confundido con X
                "L": "1IT7",    # L confundido con 1, I, T, 7
                "M": "N",       # M confundido con N
                "N": "M",       # N confundido con M
                "O": "0DCQ",    # O confundido con 0, D, C, Q
                "P": "FR",      # P confundido con F, R
                "Q": "O0G9",    # Q confundido con O, 0, G, 9
                "R": "B8P",     # R confundido con B, 8, P
                "S": "5Z2",     # S confundido con 5, Z, 2
                "T": "17JI",    # T confundido con 1, 7, J, I
                "V": "Y",       # V confundido con Y
                "W": "VY",      # W confundido con V, Y
                "X": "K",       # X confundido con K
                "Y": "VW",      # Y confundido con V, W
                "Z": "2S5"      # Z confundido con 2, S, 5
            },
            
            # ACTUALIZADO: Caracteres prohibidos en placas españolas modernas
            "forbidden_chars": "AEIÑOQUÜ",  # Vocales y caracteres especiales no usados
            
            # NUEVO: Secuencias específicas españolas que deben corregirse
            "spanish_corrections": {
                # Correcciones específicas para placas españolas
                "LL": "11",     # LL frecuentemente mal leído como 11
                "II": "11",     # II frecuentemente mal leído como 11
                "OO": "00",     # OO frecuentemente mal leído como 00
                "QQ": "00",     # QQ frecuentemente mal leído como 00
                "BB": "88",     # BB frecuentemente mal leído como 88
                "DD": "00",     # DD frecuentemente mal leído como 00
                "GG": "66",     # GG frecuentemente mal leído como 66
                "SS": "55",     # SS frecuentemente mal leído como 55
                "TT": "77",     # TT frecuentemente mal leído como 77
                "ZZ": "22",     # ZZ frecuentemente mal leído como 22
                
                # Correcciones de transiciones comunes
                "1L": "11",     # 1L -> 11
                "L1": "11",     # L1 -> 11
                "0O": "00",     # 0O -> 00
                "O0": "00",     # O0 -> 00
            ...
            # NUEVO: Validaciones específicas para placas españolas
            "validation_rules": {
                "min_length": 5,
                "max_length": 9,
                "must_have_digits": True,
                "must_have_letters": True,
                "consecutive_same_char_limit": 3,  # Máximo 3 caracteres iguales seguidos
                "invalid_sequences": ["000", "111", "222", "333", "444", "555", "666", "777", "888", "999"],
                "common_prefixes": ["A", "B", "C", "D", "F", "G", "H", "J", "K", "L", "M", "N", "P", "R", "S", "T", "V", "W", "X", "Y", "Z"],
                "regional_codes": {
                    # Códigos de provincia (para validación contextual)
                    "A": "Alicante", "AB": "Albacete", "AL": "Almería", "AV": "Ávila",
                    "B": "Barcelona", "BA": "Badajoz", "BI": "Bizkaia", "BU": "Burgos",
                    "C": "A Coruña", "CA": "Cádiz", "CC": "Cáceres", "CO": "Córdoba", "CR": "Ciudad Real", "CS": "Castellón", "CU": "Cuenca",
                    "GC": "Las Palmas", "GI": "Girona", "GR": "Granada", "GU": "Guadalajara",
                    "H": "Huelva", "HU": "Huesca",
                    "J": "Jaén",
                    "L": "Lleida", "LE": "León", "LO": "La Rioja", "LU": "Lugo",
                    "M": "Madrid", "MA": "Málaga", "MU": "Murcia",
                    "NA": "Navarra",
                    "O": "Asturias", "OR": "Ourense",
                    "P": "Palencia", "PM": "Illes Balears", "PO": "Pontevedra",
                    "S": "Cantabria", "SA": "Salamanca", "SE": "Sevilla", "SG": "Segovia", "SO": "Soria", "SS": "Gipuzkoa",
                    "T": "Tarragona", "TE": "Teruel", "TF": "Santa Cruz de Tenerife", "TO": "Toledo",
