...
def test_ocr_best_moment(video_path, output_folder, roi_points):
    print(f"🚀 Iniciando Test OCR V4 CORREGIDO: {video_path}")
        ...
        for det in detections:
            ...
            if 0.1 <= ppi <= 1.8:
                ...
                if pqi > vehicles_data[found_id]['best_pqi']:
                    ...
                    # Ejecutar OCR con V4_CORREGIDO + Autocrop Quirúrgico
    ...
    print(f"📊 Generando reportes V4 para {len(vehicles_data)} vehículos...")
    ...
    for tid, v in vehicles_data.items():
        ...
        cv2.putText(mural, f"LECTURA V4: {data['text']}", (600, 380), 
                    cv2.FONT_HERSHEY_DUPLEX, 1.2, (0, 255, 0), 2)
        cv2.putText(mural, f"Confianza: {data['ocr_conf']:.2f}", (600, 420), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 200, 200), 1)
        cv2.putText(mural, f"PPI: {data['ppi']:.2f} (Oro)", (600, 450), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 1)
        
        # Título
        cv2.putText(mural, f"TEST OCR V4 - VEHICULO #{tid}", (50, 35), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)

        output_path = os.path.join(output_folder, f"ocr_v4_result_v{tid}.png")
        cv2.imwrite(output_path, mural)
        print(f"✅ Reporte V4 guardado: {output_path} -> {data['text']}")
...
if __name__ == "__main__":
    test_ocr_best_moment("videos/VID2COLISEO.MOV", "tests/resultado_ocr_v4", ROI_COLISEO)
