#!/usr/bin/env bash
# InfractiVision ONLINE installer - Linux (per-user, XDG) — build unico CUDA con fallback CPU
...
usage(){ echo "Usage: $0 [--prefix DIR] [--with-demo|--no-demo]"; exit 0; }
while [[ $# -gt 0 ]]; do case "$1" in
  --cpu|--cuda|--auto)
    echo "[!] Flag legacy $1 ignorado: build unico CUDA con fallback CPU" >&2
    shift;;
  --with-cuda-pip|--no-cuda-pip|--without-cuda-pip)
    echo "[!] Flag legacy $1 ignorado: ya no hay pip on-demand, CUDA viene compilado" >&2
    shift;;
...
main(){
  ...
  check_deps
  # Paso 1: deteccion solo informativa — el binario es unico y decide en runtime.
  if has_nvidia_gpu; then
    echo "[*] GPU NVIDIA detectada -> el build unico usará GPU (arch=$ARCH, prefix=$PREFIX)"
  else
  ...
  # Paso 3: videos demo (descarga al directorio de datos del usuario, que es donde
...
DESKTOP
  ...
  if has_nvidia_gpu; then
    echo "  GPU: NVIDIA detectada -> usará GPU (fallback a CPU si falla)"
  else echo "  GPU: No detectada -> mismo build corriendo en CPU"; fi
}
